...
class GNN(MessagePassing):
    ...
    @staticmethod
    def norm(edge_index, num_nodes, x,improved=False, dtype=None):  #x =  allnode_f
         
        
        edge_weight = torch.ones((edge_index.size(1),), dtype=dtype,  
                                 device=edge_index.device)  #首先为每条边分配权重，默认为1。 len(edge_weight) 9936358(k = 4)
    ...
    def forward(self, x, edge_index,vars=None): #x = prompt.cat(node_f)  21530*128
        if vars is None:
            vars = self.vars
        improved = False
        ...
        self.norm_edge = norm
        x = self.propagate(edge_index, x=x, norm=self.norm_edge)#, norm=norm
        x = F.linear(x, w, b)
        x = F.leaky_relu(x)
       ...
       # x = self.propagate(edge_index, x=x, norm=norm)#, norm=norm
        x = F.linear(x, w, b)
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND, batch_size * context_length *emb_dim -> context_length * batch_size  *emb_dim
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD, context_length * batch_size *emb_dim -> batch_size * context_length *emb_dim
    ...
    def forward(self, x, adj, s_n, t_n, s_n_text, t_n_text, device, training=True):
        ...
        t_text_features = self.encode_text(t_n_text)  #目标节点上下文邻居节点
...
def tokenize(texts: Union[str, List[str]], context_length: int = 128, truncate: bool = True) -> torch.LongTensor:
    ...
    all_tokens = [[sot_token] + _tokenizer.encode(text) + [eot_token] for text in texts] #列表套列表
    #print(all_tokens[0])
